...
#ifndef AR7100_SPI_CONTROL_DEFAULT
#define AR7100_SPI_CONTROL_DEFAULT 0x43
#endif
...
static ulong read_id(void){
	...
	flashid = ar7100_reg_rd(AR7100_SPI_RD_STATUS);
...
unsigned long flash_init(void){
	...
	pll_magic = ar7100_reg_rd(CFG_FLASH_BASE + PLL_IN_FLASH_DATA_BLOCK_OFFSET + PLL_IN_FLASH_MAGIC_OFFSET);

	// read SPI CONTROL Configuration register (SPI_CONTROL) value stored in FLASH (PLL_IN_FLASH_MAGIC_OFFSET + 12)
	spi_control = ar7100_reg_rd(CFG_FLASH_BASE + PLL_IN_FLASH_DATA_BLOCK_OFFSET + PLL_IN_FLASH_MAGIC_OFFSET + 12);
#endif

	info = &flash_info[0];

	// spi flash clock
	ar7100_reg_wr(AR7100_SPI_FS,	0x01);

	// if reset button is pressed -> write default CLOCK_DIVIDER for SPI CLOCK
	if(reset_button_status()){
		ar7100_reg_wr(AR7100_SPI_CLOCK, AR7100_SPI_CONTROL_DEFAULT);
	} else {
#if defined(PLL_IN_FLASH_MAGIC_OFFSET)
		// do we have PLL_MAGIC in FLASH?
		if(pll_magic == PLL_IN_FLASH_MAGIC){
			ar7100_reg_wr(AR7100_SPI_CLOCK,	spi_control);
		} else {
#endif
			ar7100_reg_wr(AR7100_SPI_CLOCK, AR7100_SPI_CONTROL_DEFAULT);
#if defined(PLL_IN_FLASH_MAGIC_OFFSET)
